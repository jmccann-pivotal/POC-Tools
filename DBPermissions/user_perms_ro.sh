...
psql -t -q -d $1 -c "select 'grant select on ' || table_schema || '.' ||table_name || ' to ' || ('$3') || ';' from information_schema.tables where table_catalog = ('$1') and table_schema = ('$2');" | psql -t -q -e -d $1 >> $LOGFILE 2>&1
